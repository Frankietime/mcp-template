...
from tropical.tui import MCPInspectorApp
...
def tui(
    ...
    url: Annotated[
        str | None,
        typer.Option("--url", "-u", help="Auto-connect to this HTTP URL on launch"),
    ] = None,
    header: Annotated[
        list[str] | None,
        typer.Option("--header", "-H", help="HTTP request headers as KEY=VALUE (repeatable)"),
    ] = None,
) -> None:
    ...
    try:
        ...
        headers_dict: dict[str, str] = _parse_key_value_list(header or [])
        console.print("[bold blue]Starting MCP Inspector TUI[/bold blue]")
        MCPInspectorApp(debug=debug, url=url, headers=headers_dict or None).run()
...
def connect_http(
    ...
    headers_dict = _parse_key_value_list(header or [])
    asyncio.run(_connect_arbitrary_http_server(url, verbose, debug, raw_interactions, debug_dump, name, headers_dict))
...
def _parse_key_value_list(pairs: list[str]) -> dict[str, str]:
    """Parse a list of ``KEY=VALUE`` strings into a dict, ignoring malformed entries."""
    result: dict[str, str] = {}
    for pair in pairs:
        if "=" in pair:
