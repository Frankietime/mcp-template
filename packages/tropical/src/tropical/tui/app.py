...
class MCPInspectorApp(App[None]):
    ...
    def __init__(self, debug: bool = False, url: str | None = None, headers: dict[str, str] | None = None) -> None:
        """Initialize the application.

        Args:
            debug: Whether to enable debug logging to file
            url: Optional HTTP URL to auto-connect on launch (e.g. from equator /tropical)
            headers: Optional HTTP headers forwarded with every request (e.g. Authorization)
        """
        super().__init__()
        ...
        self._autoconnect_headers = headers or {}
    ...
    def on_mount(self) -> None:
        ...
        if self._autoconnect_url:
            self.set_timer(0.5, self._do_autoconnect)

    @work
    async def _do_autoconnect(self) -> None:
        """Auto-connect to the URL supplied at launch (e.g. forwarded from equator /tropical)."""
        ...
        server = MCPServer(
            ...
            http_headers=self._autoconnect_headers or None,
        )
        try:
            await self.mcp_service.connect(server)
        except Exception as e:
            self.notify_error(f"Auto-connect failed: {e}")
