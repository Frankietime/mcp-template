...
class TestContextBarStyle:
    def test_below_70_percent_is_low(self) -> None:
        frags = _fragments(10_000, 32_768)  # ~30%
        bar_style = next(style for style, text in frags if _FILL in text or _EMPTY in text)
        assert bar_style == "class:ctx.low"

    def test_above_70_percent_is_mid(self) -> None:
        frags = _fragments(25_000, 32_768)  # ~76%
        bar_style = next(style for style, text in frags if _FILL in text or _EMPTY in text)
        assert bar_style == "class:ctx.mid"

    def test_above_90_percent_is_high(self) -> None:
        frags = _fragments(31_000, 32_768)  # ~95%
        bar_style = next(style for style, text in frags if _FILL in text or _EMPTY in text)
        assert bar_style == "class:ctx.high"
